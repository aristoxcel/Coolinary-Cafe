        ...
        <div onClick={() => setOpenModal(false)} className={`fixed z-[100] flex items-center justify-center ${openModal ? 'opacity-1 visible' : 'invisible opacity-0'} inset-0 h-full w-full bg-black/20 backdrop-blur-sm duration-100`}>
          <div onClick={(e_) => e_.stopPropagation()} className={`absolute w-full rounded-lg bg-white dark:bg-gray-900 drop-shadow-2xl sm:w-[500px] ${openModal ? 'opacity-1 translate-y-0 duration-300' : '-translate-y-20 opacity-0 duration-150'}`}>
            <form onSubmit={handleFeedback} className="px-5 pb-5 pt-3 lg:pb-10 lg:pt-5 lg:px-10">
              ...
              <h1 className="text-4xl backdrop-blur-sm">Your Highness</h1>
              <h2 className="pb-8 text-xl backdrop-blur-sm pl-2">Give us Your Feedback</h2>
              <div className="space-y-5">
                ...
                <div className="relative">
                  ...
                  disabled 
                  className="block w-full rounded-lg p-3 pl-10 outline-none  bg-white dark:bg-gray-700 dark:text-white" />
                ...
                <textarea
                      className="block w-full rounded-lg p-3 pl-10 outline-none drop-shadow-lg bg-white dark:bg-gray-700 dark:text-white"
                      rows={2}
                      placeholder="Feedback / Comment" name="comment"
                      />
                      <span className="absolute left-2 top-1/4">
                  <FaPenToSquare  className="text-xl  ml-1"/>
                  </span>
                </div>
              </div>
